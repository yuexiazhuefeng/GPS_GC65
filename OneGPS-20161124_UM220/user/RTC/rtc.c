...
u8 RTC_Init(void)
	...
	//	BKP_DeInit();	//将外设BKP的全部寄存器重设为缺省值 	
		...
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		RTC_Set1(2012,12,2,10,0,55);  //设置时间	  
...
u8 RTC_Set1(u16 syear,u8 smon,u8 sday,u8 hour,u8 min,u8 sec)
	{
	u16 t;
	u32 seccount=0;
...
//输入： 所有的参数都是16进制数
//输出： 成功：0，晶振出错：1，输入参数出错：2
u8 RTC_Set(u16 syear,u8 smon,u8 sday,u8 hour,u8 min,u8 sec)
{
	//检查是不是第一次配置时钟
	u8 temp=0;
	if ((smon>12)|| (sday>31)||(hour >23)||(min >60) ||(sec >60))
		return 2;
	...
	if ((smon==2) ||(smon==4) ||(smon==6) ||(smon==9) || (smon==11))
	{
		 if(sday >30)
		    return 2;	
	}
...
//	if (BKP_ReadBackupRegister(BKP_DR1) != 0x5050)		//从指定的后备寄存器中读出数据:读出了与写入的指定数据不相乎
	...
	//	BKP_DeInit();	//将外设BKP的全部寄存器重设为缺省值 	
		...
		RTC_SetPrescaler(32767); //设置RTC预分频的值
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		RTC_Set1(syear,smon,sday,hour,min,sec);  //设置时间	  
		BKP_WriteBackupRegister(BKP_DR1, 0X5050);	//向指定的后备寄存器中写入用户程序数据
		}
//	else//系统继续计时
//		{
